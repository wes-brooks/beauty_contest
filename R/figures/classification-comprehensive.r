...
for (method in methods) {
    for (site in sites) {
        ...
        for (f in ra$fold %>% unique %>% sort) {
            ...
            pp.comp = rbind(pp.comp, c(tp, tn, fp, fn, thresh))
        }
        
        #add an aggregation row and put row, column names on the perf data:
        pp.comp = rbind(pp.comp, colSums(pp.comp))
        colnames(pp.comp) = c('tp', 'tn', 'fp', 'fn', 'thresh')
        rownames(pp.comp) = c('1', '2', '3', '4', 'tot')
        
        #return this perf data to the main result object:
        perf[[method]][[site]] = pp.comp
    }
}
...
#plot the results:
pp.comprehensive = list()
for (s in sites) {
    yrange = max(filter(perf, site==s)$count)
    
    pp.comprehensive[[s]] = ggplot(filter(perf, site==s)) +
        ...
        ylim(c(0,1.1*yrange)) +
        facet_grid(.~method, labeller=function(x, j) return(pretty.methods[sort(methods)])) +
    ...
    pp.comprehensive[[s]] = pp.comprehensive[[s]] +
        geom_text(
            aes(x=rep(c(1.775, 0.775, 1.225, 2.225), 14),
                y=count + yrange/50,
                label=count),
            hjust=0.5,
            vjust=0
