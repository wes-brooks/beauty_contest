...
COMMENT: All units need to be converted to metric. The above paragraph has inches, and distances in the site descriptions use miles. It would pay to check this throughout the manuscript.

Additional covariates were compiled from a variety of sources including online data and manual measurements. Online data were accessed using Environmental Data Discovery and Transformation (EnDDaT), a web service that accesses data from a variety of sources, compiles and processes the data, and performs common transformations [@EnDDaT-2014]. Three sources of data were accessed: The U.S. Geological survey National Water Information System (NWIS), the National Weather Service North Central River Forecasting Center (NCRFS), and the National Oceanographic and Atmospheric Administration Great Lakes Costal Forecasting System (GLCFS).  Covariates acquired through these sources included: river discharge, precipitation, lake current vectors, wave height, wave direction, lake level, water temperature, air temperature, wind vector, and percent cloud cover.

Comment: we will need references for NWIS, NCRFS, and GLCFS. 
GLCFS: Schwab, D. J.; Bedford, K.W. Great Lakes forecasting. In Coastal Ocean Prediction; Mooers C., Ed.; Amer. Geophys. Union Coastal and Estuarine Studies, 1999.

NWIS: http://waterdata.usgs.gov/nwis

NCRF: request in to Dave Blodgett for this.
...
\end{align}
where $\hat{\sigma}^2$ was the variance estimate from the model that used all of the covariates, $n$ was the sample size, $y_i$ and $\hat{y}_i$ were respectively the observed and fitted value of the $i$th FIB measurement, and $df$ was the number of covariates in the model. For binary regression models, the $\text{AIC}_{\tt c}$ was calculated by
...
Here, the GA was used to select covariates for either an OLS or a logistic regression model. By analogy to natural selection, so-called chromosomes in the GA represent regression models [@Fogel-1998]. A covariate was included in the model if the corresponding element of the chromosome was one, but not otherwise. Chromosomes were produced in successive generations, where the first generation was produced randomly and subsequent generations were produced by combining chromosomes from the current generation, with additional random drift. The chance that a chromosome in the current generation will produce offspring in the next generation was an increasing function of its fitness. The fitness of each chromosome was calculated by the $\text{AIC}_{\tt c}$ (\ref{eq:AICc-continuous}), (\ref{eq:AICc-binary}).
...
## AUROC
COMMENT: Definition of the ROC curve, sensitivity, and specificity are probably more appropriate to move into methods.
START HERE FOR MOVING AUROC DESCRIPTION TO METHODS

The ROC curve is an assessment of how well predictions were separated into exceedances and nonexceedances [@Hanley-McNeil-1982]. Every possible value of the decision threshold $\hat{\delta}$ corresponds to the point on the ROC curve with coordinates $(1-\text{specificity}, \text{sensitivity})$. Specificity is the fraction of decision threshold non-exceedances that have been correctly predicted.Sensitivity is the fraction of decision threshold exceedances that have been correctly predicted. Specificity and sensitivity are mathematically defined as 
...
COMMENT: END HERE FOR MOVING AUROC DESCRIPTION TO METHODS SECTION
...
COMMENT: PRESS SHOULD ALSO BE DEFINED IN METHODS. SOME OF THIS EXPLANATION (COMPARISON OF PROS AND CONS AND COMPARISON TO AUROC) STILL BELONGS HERE, BUT THE ACTUAL DEFINITION OF PRESS SHOULD BE LOCATED IN METHODS.
