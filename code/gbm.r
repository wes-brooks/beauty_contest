...
Model = list( 
    ...
    Create = function(self, ...) {
		...
		} else { #If there is no 'weights' key, set all weights to one.
    ...
    Extract = function(self, model_part, ...) {
        ...
        if ('extract_from' in %in% names(args)) {
    ...
    Predict = function(self, data, **args) {
    ...
    GetFitted = function(self, ...) {
        self[['fitted']] = predict(object=self[['model']], n.trees=self[['trees']], newdata=self[['data_frame']], ...)
    ...
    Threshold = function(self, specificity=0.9) {
        ...
        error = {
			return(self[['regulatory_threshold']])
		})
	}
)